...
// Per-stage real enemy anm sprite sizes (generated by compare.sh from the port's
// enemy_anm_sizes example). Defines anm_enemy_size(id, &w, &h); a default no-op
// version is provided when no table was generated.
#include "enemy_sizes.h"
namespace th06 {
#pragma once
...
struct AnmManager {
    ...
    void SetAndExecuteScriptIdx(AnmVm* vm, i32 scriptIdx){
        ...
        // Enemy main-anm scripts (ANM_SCRIPT_ENEMY_START 0x100 .. 0x1a5): the
        // despawn (EnemyManager.cpp:549) reads this sprite size, so it must be the
        // REAL per-stage value (e.g. stage-1 fairies 30x30, stage-5 30x32, Rumia
        // 32x48), not a single hardcoded guess. anm_enemy_size() is the generated
        // per-stage table; fall back to 30x32 if a script is missing from it.
        if (scriptIdx >= 0x100 && scriptIdx <= 0x1a5) {
            i32 i = scriptIdx & 1023;
            f32 w = 30, h = 32;
            anm_enemy_size(scriptIdx, &w, &h);
            spritePool[i].widthPx = w; spritePool[i].heightPx = h;
            vm->sprite = &spritePool[i];
